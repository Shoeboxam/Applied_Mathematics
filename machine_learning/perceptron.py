...
def numpy_example():
    ...
    for i in range(iterations):
        # I'm using an implicit broadcast on last dimension of b
        ...
        print('\nIteration: ' + str(i + 1))
        print(f'dl_dW = {dl_dW} = ({W} @ {X} + {b} - {Y}) @ {X}.T / {X.shape[1]}')
        print('W = ' + str(W))

        print(f'dl_db = {dl_db} = ∑({W} @ {X} + {b} - {Y}) / {X.shape[1]}')
        print('b = ' + str(b))

        print(f"Error {i + 1}: {np.sum(((W @ X + b) - Y) ** 2 / (2 * X.shape[1]))}")

        W -= learn_rate * dl_dW
        b -= learn_rate * dl_db

    print(f"\nPrediction after 5 steps with γ = {learn_rate}:")
    print(W @ X + b)
